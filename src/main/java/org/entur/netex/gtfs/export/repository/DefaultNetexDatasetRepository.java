...
public class DefaultNetexDatasetRepository implements NetexDatasetRepository {
  ...
  @Override
  public OperatingDay getOperatingDayByDayTypeAssignment(
    DayTypeAssignment dayTypeAssignment
  ) {
    if (dayTypeAssignment.getOperatingDayRef() == null) {
      throw new GtfsExportException(
        "OperatingDay undefined for DayTypeAssignment " +
        dayTypeAssignment.getId()
      );
    }
    OperatingDay operatingDay = netexEntitiesIndex
      .getOperatingDayIndex()
      .get(dayTypeAssignment.getOperatingDayRef().getRef());
    if (operatingDay == null) {
      throw new GtfsExportException(
        "OperatingDay not found: " + dayTypeAssignment.getOperatingDayRef()
      );
    }
    return operatingDay;
  }
  ...
  @Override
  public OperatingPeriod getOperatingPeriodByDayTypeAssignment(
    DayTypeAssignment dayTypeAssignment
  ) {
    if (dayTypeAssignment.getOperatingPeriodRef() == null) {
      throw new GtfsExportException(
        "OperatingPeriod undefined for DayTypeAssignment " +
        dayTypeAssignment.getId()
      );
    }
    ...
    if (operatingPeriod == null) {
      throw new GtfsExportException(
        ...
        dayTypeAssignment.getOperatingPeriodRef()
      );
    }
  ...
  @Override
  public DayType getDayTypeByDayTypeAssignment(
    DayTypeAssignment dayTypeAssignment
  ) {
  ...
  public String getTimeZone() {
    if (timezone == null) {
      Set<String> timeZones = netexEntitiesIndex
        .getCompositeFrames()
        .stream()
      ...
      if (timeZones.size() > 1) {
        throw new DefaultTimeZoneException(
          "The dataset contains more than one default timezone"
        );
      }
      timezone =
        timeZones
          ...
          .findFirst()
          .orElseThrow(() ->
            new DefaultTimeZoneException(
              "The dataset does not contain a default timezone"
            )
          );
    }
    return timezone;
  ...
  private Network findNetwork(String networkOrGroupOfLinesRef) {
    Network network = netexEntitiesIndex
      .getNetworkIndex()
      .get(networkOrGroupOfLinesRef);
    if (network != null) {
      return network;
    } else {
      return netexEntitiesIndex
        ...
        .filter(n ->
          n
            ...
            .getGroupOfLines()
            .stream()
            .anyMatch(groupOfLine ->
              groupOfLine.getId().equals(networkOrGroupOfLinesRef)
            )
        )
        .findFirst()
        .orElseThrow(() ->
          new GtfsExportException(
            "Could not find Network for network or group of lines id " +
            networkOrGroupOfLinesRef
          )
        );
    }
  }
  ...
  @Override
  public Collection<DayTypeAssignment> getDayTypeAssignmentsByDayType(
    DayType dayType
  ) {
    return netexEntitiesIndex
  ...
  @Override
  public String getFlexibleStopPlaceIdByScheduledStopPointId(
    String scheduledStopPointId
  ) {
    return netexEntitiesIndex
      .getFlexibleStopPlaceIdByStopPointRefIndex()
      .get(scheduledStopPointId);
  }

  @Override
  public String getQuayIdByScheduledStopPointId(String scheduledStopPointId) {
    String quayId = netexEntitiesIndex
      .getQuayIdByStopPointRefIndex()
      .get(scheduledStopPointId);
    if (quayId == null) {
      throw new QuayNotFoundException(
        "Could not find Quay id for scheduled stop point id " +
  ...
  @Override
  public Collection<ServiceJourney> getServiceJourneysByJourneyPattern(
    ...
    return getServiceJourneys()
      ...
      .filter(serviceJourney ->
        serviceJourney
          ...
          .getValue()
          .getRef()
          .equals(journeyPattern.getId())
      )
      .collect(Collectors.toSet());
  }

  @Override
  public Collection<org.rutebanken.netex.model.Route> getRoutesByLine(
    Line line
  ) {
    return netexEntitiesIndex
      .getRouteIndex()
      ...
      .stream()
      .filter(route ->
        route.getLineRef().getValue().getRef().equals(line.getId())
      )
      .collect(Collectors.toSet());
  }

  @Override
  public Collection<JourneyPattern> getJourneyPatternsByRoute(
    org.rutebanken.netex.model.Route route
  ) {
    return netexEntitiesIndex
      ...
      .filter(journeyPattern ->
        journeyPattern.getRouteRef().getRef().equals(route.getId())
      )
      .collect(Collectors.toSet());
  }
  ...
  @Override
  public DestinationDisplay getDestinationDisplayById(
    String destinationDisplayId
  ) {
    return netexEntitiesIndex
      .getDestinationDisplayIndex()
      .get(destinationDisplayId);
  }

  @Override
  public Collection<DatedServiceJourney> getDatedServiceJourneysByServiceJourneyId(
  ...
  public DayType getDayTypeById(String dayTypeId) {
    ...
    if (dayType == null) {
      throw new GtfsExportException(
        "Could not find DayType with id " + dayTypeId
      );
    }
    return dayType;
  }
  ...
  public OperatingDay getOperatingDayById(String operatingDayId) {
    ...
    if (operatingDay == null) {
      throw new GtfsExportException(
        "Could not find OperatingDay with id " + operatingDayId
      );
    }
    return operatingDay;
  }
